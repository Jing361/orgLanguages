...
class EmployeeList extends ArrayList<Employee>{
    EmployeeList() { }
    // Return Employee object whose name matches 'nm' or null if none exists.
    Employee find(String nm) {
        int i = 0;
        Employee temp = this.get(0);
        while(temp.name != nm){
            ++i;
            temp = this.get(i);
        }
    ...
    void setHours(String nm, double hrs) { this.find(nm).setHours(hrs); }
    void setSalary(String nm, double salary) { this.find(nm).setSalary(salary); }
    void setSales(String nm, double sales) { this.find(nm).setSales(sales); }
    double payroll() { 
        double roll = 0;
        for(int i = 0; i < this.size(); ++i){
            roll += this.get(i).computePay();
        }
        return roll;
    }
    void display() {
    ...
    boolean enqueue(Employee e) { return this.add(e); }
}
